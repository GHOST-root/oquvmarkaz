  ...
  return (
    <div className="dashboard-card mb-4" style={{ animation: 'fadeInDown 0.3s ease' }}>
      <div className="d-flex justify-content-between align-items-center mb-3 border-bottom pb-3">
        <div><h5 className="m-0 text-dark fw-semibold">Joriy oyda to'lov qilgan talabalar</h5><span className="text-muted small">Miqdor: {students.length} ta to'lov</span></div>
          ...
          <tbody>
            {sortedStudents.length > 0 ? sortedStudents.map((student, index) => (
              <tr key={student.id || index}>
                ...
                <td className="text-success fw-medium">{formatMoney(student.paymentAmount || 450000)}</td>
                <td className="text-muted"><i className="fa-regular fa-calendar me-2"></i>{student.paymentDate || '15.02.2025'}</td>
              </tr>
            )) : <tr><td colSpan="5" className="text-center py-4 text-muted">Bu oyda hozircha to'lovlar yo'q.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
