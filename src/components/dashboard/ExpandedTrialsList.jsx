  ...
  return (
    <div className="dashboard-card mb-4" style={{ animation: 'fadeInDown 0.3s ease' }}>
      <div className="d-flex justify-content-between align-items-center mb-3 border-bottom pb-3">
        <div><h5 className="m-0 text-dark fw-semibold">Sinov darsidagi talabalar ro'yxati</h5><span className="text-muted small">Miqdor: {students.length} ta</span></div>
          ...
          <tbody>
            {sortedStudents.length > 0 ? sortedStudents.map(student => (
              <tr key={student.id}>
                ...
                <td><span className="badge bg-warning bg-opacity-10 text-warning rounded-pill px-3">{student.status}</span></td>
              </tr>
